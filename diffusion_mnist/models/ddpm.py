...
class DDPMGenerator():
    ...
    def sample_with_context(self, x, ctx=None, grad=True):
        """
        x : (batch, 1, 28, 28) : initial latents (noise)
        ctx: (batch, 1) : classes of the images to generate
        """

        ctx_encoded = F.one_hot(ctx, 10).to(hp.DEVICE).float() # (batch, 10)
        ...
        for i in range(hp.timesteps, 0, -1):
            ...
            t = torch.tensor([i / hp.timesteps])[:, None, None, None].to(hp.DEVICE) # (batch_size,1,1,1)
            z = torch.randn_like(samples) if i > 1 else 0

            with torch.set_grad_enabled(grad):
                eps = self.model(samples, t, ctx_encoded)    # predict noise e_(x_t, t, ctx)
            
            samples = self._denoise(samples, i, eps, z)

        print(f'Finisehd generating batch with shape {samples.shape}')
        return samples
    
    # helper function; calculates x_{t-1} from x_t by removing the predicted noise and adding back in the scaled noise (to avoid collapse)
    ...
    # def sample_with_context_and_intermediate(self, x, ctx=None, grad=True):
    #     """
    #     x : (batch, 1, 28, 28) : initial latents (noise)
    #     ctx: (batch, 1) : classes of the images to generate
    #     """
